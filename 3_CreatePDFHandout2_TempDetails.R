...
pdf("OUTPUT/HainesJct_EnvData_Handout2_TempDetails_HainesJct.pdf",onefile=TRUE, height=8.5,width=11)
...
min.idx <- merged.df[,"Station"]== "HainesJct"  & merged.df[,"Variable"]== "AirTemp_Min" & merged.df[,"Month"]== month
min.vec <- merged.df[min.idx,"Value"]
min.dates <- merged.df[min.idx,"FullDate"]
...
max.idx <- merged.df[,"Station"]== "HainesJct"  & merged.df[,"Variable"]== "AirTemp_Max" & merged.df[,"Month"]== month
max.vec <- merged.df[max.idx,"Value"]
max.dates <- merged.df[max.idx,"FullDate"]


x.lim <- range(merged.df[max.idx | min.idx | mean.idx ,"FullDate"])
y.lim <- range(merged.df[max.idx | min.idx | mean.idx ,"Value"],na.rm=TRUE)
